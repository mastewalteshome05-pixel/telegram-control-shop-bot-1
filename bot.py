...
def get_safe_connection():
    ...
    for _ in range(3):
        try:
            ...
            last_err = e
            logger.warning("🔄 Re-initializing connection pool...")
            with db_pool_lock:
                try:
                    db_pool.closeall()
                except Exception:
...
CONTROL_STRINGS = {
    ...
    "en": {
        "welcome": "👋 Welcome to Store Registration Bot!\n\n📌 **To register a new store:**\n1️⃣ Create a bot with @BotFather using `/newbot`\n2️⃣ Click '📝 Register New Store' after receiving the token\n3️⃣ Complete 5 steps\n\n📌 **View your stores:** 🏪 My Stores\n\n👑 **If you're Super Admin:** `/superadmin`",
...
def setup_bot_handlers(token):
    ...
    def finalize_checkout(chat_id, lang, edit_call=None):
        ...
        try:
            ...
            bot.send_message(chat_id, "❌ ስህተት ተከስቷል!")
            return
        finally:
            put_conn(conn)
            with user_carts_lock:
    ...
    def handle_incoming_photos(message):
        ...
        if state.startswith("AWAITING_RECEIPT_"):
            ...
            photo_id = message.photo[-1].file_id
            with admin_states_lock:
                p_data = admin_states.get(session_key, {}).get("data", {})
    ...
    @bot.message_handler(func=lambda m: admin_states.get((token, m.chat.id), {}).get("state") == "WAITING_PRODUCT_PHOTO" and m.text and m.text.lower() in ['none', 'ስቀር', 'skip'])
    ...
    def process_edit_product(message):
        ...
        if not is_verified_admin(chat_id):
            return
        with admin_states_lock:
            p_id = admin_states.get(session_key, {}).get("data", {}).get("product_id")
        if not p_id:
    ...
    def handle_global_ai(message):
        ...
        try:
            ...
            bot.reply_to(message, response.text[:1000] if response.text else "❌ ምላሽ አልተገኘም")
        except Exception as e:
            logger.error(f"AI error: {e}")
    ...
    threading.Thread(target=_run_bot, name=f"Bot_{token[:15]}", daemon=True).start()
...
def load_existing_stores_from_db():
    ...
    count = 0
    for (tok,) in rows:
        if start_shop_bot(tok):
            count += 1
    logger.info(f"✅ {count} approved stores restored.")

load_existing_stores_from_db()

# ============================================================
# 7. CONTROL BOT WITH SUPER ADMIN PANEL - FIXED
# ============================================================
...
if CONTROL_BOT_TOKEN:
    ...
    super_admin_sessions_lock = threading.Lock()
    super_login_attempts = {}
    super_login_attempts_lock = threading.Lock()
    reg_states = {}
    reg_states_lock = threading.Lock()

    def is_super_admin(chat_id):
        with super_admin_sessions_lock:
            return chat_id in super_admin_sessions and time.time() < super_admin_sessions[chat_id]

    # ============================================================
    # 7.1 CONTROL BOT MAIN MENU - FIXED
    # ============================================================
    
    def get_control_main_menu(lang="am"):
        cs = CONTROL_STRINGS[lang]
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
        markup.add(
            types.KeyboardButton(cs["register"]),
            types.KeyboardButton(cs["my_stores"])
        )
        markup.add(
            types.KeyboardButton(cs["search"]),
            types.KeyboardButton(cs["help"])
        )
        return markup
    ...
    def super_auth_start(message):
        ...
        with super_login_attempts_lock:
    ...
    # ============================================================
    # 7.9 CONTROL BOT USER COMMANDS - FIXED
    # ============================================================
    ...
    def control_help(message):
        ...
        control_bot.send_message(
            chat_id,
            cs["welcome"],
            reply_markup=get_control_main_menu(lang),
            parse_mode="Markdown"
        )

    @control_bot.message_handler(func=lambda m: m.text == CONTROL_STRINGS["am"]["register"] or m.text == CONTROL_STRINGS["en"]["register"])
    def start_registration(message):
        chat_id = message.chat.id
        
        with reg_states_lock:
    ...
    def reg_step_token(message):
        ...
        with reg_states_lock:
            if chat_id not in reg_states:
                reg_states[chat_id] = {"step": 2, "data": {}}
    ...
    def reg_step_name(message):
        ...
        with reg_states_lock:
            if chat_id not in reg_states:
                reg_states[chat_id] = {"step": 3, "data": {}}
    ...
    def reg_step_password(message):
        ...
        if len(password) < 8:
            control_bot.reply_to(message, "❌ ቢያንስ 8 ፊደል!")
            return
        
        with reg_states_lock:
            if chat_id not in reg_states:
                reg_states[chat_id] = {"step": 4, "data": {}}
    ...
    def reg_step_location(message):
        ...
        with reg_states_lock:
            if chat_id not in reg_states:
                reg_states[chat_id] = {"step": 5, "data": {}}
    ...
    def reg_step_description(message):
        ...
        with reg_states_lock:
            if chat_id not in reg_states:
                control_bot.reply_to(message, "❌ ስህተት! እንደገና ይሞክሩ")
                return
        ...
        try:
            with conn.cursor() as cursor:
                ...
                control_bot.reply_to(
                    ...
                    reply_markup=get_control_main_menu("am")
                )
        except Exception as e:
    ...
    @control_bot.message_handler(func=lambda m: m.text == CONTROL_STRINGS["am"]["my_stores"] or m.text == CONTROL_STRINGS["en"]["my_stores"])
    def show_my_stores_control(message):
        ...
        control_bot.reply_to(message, text, reply_markup=get_control_main_menu("am"))

    @control_bot.message_handler(func=lambda m: m.text == CONTROL_STRINGS["am"]["search"] or m.text == CONTROL_STRINGS["en"]["search"])
    ...
    def process_control_search_name(message):
        ...
        if not stores:
            control_bot.reply_to(message, "🔍 ምንም ሱቅ አልተገኘም", reply_markup=get_control_main_menu("am"))
            return
    ...
    def handle_control_location_search(message):
        ...
        if not stores:
            control_bot.reply_to(message, "🔍 በአቅራቢያ ምንም ሱቅ አልተገኘም", reply_markup=get_control_main_menu("am"))
            return
    ...
    @control_bot.message_handler(func=lambda m: m.text == CONTROL_STRINGS["am"]["help"] or m.text == CONTROL_STRINGS["en"]["help"])
